...
class RankBox(ft.UserControl):
    ...
    def __init__(self):
        super().__init__()
        self.rankText = ft.Text("Rank",
                                color=ft.colors.ON_TERTIARY_CONTAINER,
                                text_align=ft.TextAlign.LEFT,
                                width=100,
                                size=20,
                                weight=ft.FontWeight.BOLD)
        self.nameText = ft.Text("Name", color=ft.colors.ON_TERTIARY_CONTAINER, expand=1, size=18)
        self.scoreText = ft.Text("Score",
                                 color=ft.colors.ON_TERTIARY_CONTAINER,
    ...
    def build(self):
        self.container = ft.Container(
            ft.Row(controls=[self.rankText, self.nameText, self.scoreText],
                   expand=1))
...
class Leaderboard(ft.UserControl):
    ...
    def populate(self):
        ...
        for rank in range(1, 11):
            ...
            self.rank_boxes[rank].nameText.value = self.leaderboard_db.all()[
                rank - 1]['name']
            self.rank_boxes[rank].scoreText.value = self.leaderboard_db.all()[
                rank - 1]['score']
    ...
    def build(self):
        ...
        self.container = ft.Container(content=ft.Column(
            ...
            horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                                      ...
                                      padding=10,
                                      margin=10,
                                      border=ft.border.all(width=3,
                                                           color=ft.colors.TERTIARY),
                                      border_radius=10,
                                      bgcolor=ft.colors.TERTIARY_CONTAINER,
                                      width=750)

        self.ld_title = ft.Text(value="Leaderboard",
